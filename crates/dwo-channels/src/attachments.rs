...
pub fn attachment_directory(profile_root: &Path, channel: &str, session_id: &SessionId) -> PathBuf {
    let now = Local::now();
    profile_root
        .join("runtime")
        .join("attachments")
...
pub async fn unique_attachment_path(directory: &Path, filename: &str) -> Result<PathBuf> {
...
pub fn local_file_resource(path: &Path, mime_type: &str) -> Result<ContentBlock> {
...
pub fn media_mime_type(path: &Path, fallback: &str) -> String {
    match path
        .extension()
