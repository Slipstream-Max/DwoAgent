...
pub fn resolve_permission_decision(decision: &str) -> Result<PermissionOutcome> {
    ...
    let (allowed, reason) = match trimmed {
        PERMISSION_ALLOW_ONCE => (true, None),
        PERMISSION_REJECT_ONCE | PERMISSION_CANCELLED => (false, None),
        other => {
            if let Some(reason) = other.strip_prefix(&format!("{PERMISSION_REJECT_ONCE}:")) {
                (
                    false,
                    Some(reason.trim().to_string()).filter(|value| !value.is_empty()),
                )
            } else {
                bail!("Invalid permission decision: {other}");
            }
        }
    };
    Ok(PermissionOutcome { allowed, reason })
}
...
mod tests {
    ...
    #[test]
    fn permission_decision_supports_rejection_reason() {
        assert_eq!(
            resolve_permission_decision(PERMISSION_ALLOW_ONCE).unwrap(),
            PermissionOutcome {
                allowed: true,
                reason: None
            }
        ...
        assert_eq!(
            resolve_permission_decision("reject_once:use read-only command").unwrap(),
            PermissionOutcome {
                allowed: false,
        ...
        assert_eq!(
            resolve_permission_decision("reject_once:   ").unwrap(),
            PermissionOutcome {
                allowed: false,
                reason: None
            }
        );
        assert!(resolve_permission_decision("approve").is_err());
    }
