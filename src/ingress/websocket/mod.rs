...
const WEBSOCKET_SECRET_DIR: &str = "channel_secret/websocket";
...
pub fn run_websocket_login(agent_folder: &Path) -> Result<()> {
    ...
    if !config.websocket.enabled {
        bail!("websocket channel is not enabled in channels.yaml.");
    }
    if !config.websocket.auth {
        bail!("websocket auth is disabled in channels.yaml; set `websocket.auth: true` first.");
    }
