...
- Inspect one channel: `dwo channel CHANNEL status`, where `CHANNEL` is `weixin`, `telegram`, `feishu`, or `qq`.
...
- Inspect WebSocket credentials with `dwo websocket token` or rotate them with `dwo websocket reset-token` only when explicitly requested. Token rotation disconnects existing clients.
