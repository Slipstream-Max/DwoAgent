...
use dwo_command::{SessionCommand as SlashCommand, parse_session_command as parse_slash_command};

use dwo_host::Host;
use dwo_ipc as ipc;
use dwo_protocol::{self as ipc_schema, RpcEvent, RpcRoute, SessionOptions};
...
#[derive(Clone)]
struct AcpRuntime {
    // Retained only for the ACP v1 stdio shim, whose adapter remains IPC-only.
    config_path: PathBuf,
    backend: AcpBackend,
...
#[derive(Clone)]
enum AcpBackend {
    Ipc { config_path: PathBuf },
    Host { host: Arc<Host>, client_id: String },
}

impl AcpBackend {
    async fn request(&self, method: &str, params: Value) -> Result<Value> {
        match self {
            Self::Ipc { config_path } => ipc::request_acp(config_path, method, params).await,
            Self::Host { host, client_id } => {
                anyhow::ensure!(
                    dwo_protocol::method_allowed("acp", method),
                    "method {method} is not available on the acp route"
                );
                host.handle_request(client_id, &Uuid::new_v4().to_string(), method, params)
                    .await
            }
        }
    }
    ...
    async fn subscribe(
        ...
        session_id: &str,
        endpoint_id: &str,
    ) -> Result<(Value, tokio::sync::mpsc::Receiver<Value>)> {
        match self {
            Self::Ipc { config_path } => {
                ...
                tokio::spawn(async move {
                    ...
                    while let Some(event) = events.recv().await {
                        let value = match serde_json::to_value(event) {
                            Ok(params) => serde_json::to_value(RpcEvent::new(
                                RpcRoute::Acp,
                                "session.event",
                                params,
                            )),
...
impl AcpRuntime {
    async fn request(&self, method: &str, params: Value) -> Result<Value> {
        self.backend.request(method, params).await
    }
    ...
    async fn subscribe(
        ...
        self.backend.subscribe(session_id, endpoint_id).await
    }
}
...
pub async fn run_with_protocol_io<R, W>(
    config_path: PathBuf,
...
pub async fn run_with_io<R, W>(config_path: PathBuf, stdin: R, stdout: W) -> Result<()>
where
    R: AsyncRead + Send + Unpin + 'static,
    W: AsyncWrite + Send + Unpin + 'static,
{
    run_with_backend_io(AcpBackend::Ipc { config_path }, stdin, stdout).await
}

pub async fn run_with_host_io<R, W>(host: Arc<Host>, stdin: R, stdout: W) -> Result<()>
where
    R: AsyncRead + Send + Unpin + 'static,
    W: AsyncWrite + Send + Unpin + 'static,
{
    run_with_backend_io(
        AcpBackend::Host {
            host,
            client_id: format!("acp-websocket-{}", Uuid::new_v4()),
        },
        stdin,
        stdout,
    )
...
async fn run_with_backend_io<R, W>(backend: AcpBackend, stdin: R, stdout: W) -> Result<()>
where
    R: AsyncRead + Send + Unpin + 'static,
    W: AsyncWrite + Send + Unpin + 'static,
{
    let config_path = match &backend {
        AcpBackend::Ipc { config_path } => config_path.clone(),
        AcpBackend::Host { .. } => PathBuf::new(),
    };
    ...
    let agent = Agent
            ...
            async move |request: NewSessionRequest,
                    ...
                    .await
                {
                    Ok(value) => {
                        ...
                        match session_config_options(&new_runtime, id).await {
                            Ok(options) => {
            ...
            async move |request: ResumeSessionRequest,
                ...
                cx.clone().spawn(async move {
                    ...
                    match prepare_observer(&runtime, &session_id, replay).await {
                        Ok(prepared) => match session_config_options(&runtime, &session_id).await {
                            Ok(options) => {
                                ...
                                if result.is_ok() {
                                    activate_observer(&runtime, &session_id, &connection, prepared)
                                        .await?;
                                    send_available_commands(&runtime, &connection, &session_id)
                                        .await;
                                }
                                result
                            }
        ...
        .on_receive_request(
            async move |request: DeleteSessionRequest,
                ...
                let session_id = request.session_id.to_string();
                match runtime
                    .request("session.delete", json!({"session_id": session_id}))
                    .await
                {
                    Ok(_) => {
                        runtime.observers.lock().await.remove(&session_id);
        ...
        .on_receive_request(
            async move |request: SetSessionConfigOptionRequest,
                ...
                match set_runtime
                    ...
                    .await
                {
                    Ok(value) => match session_config_options(&set_runtime, &session_id).await {
                        Ok(options) => {
                            let result =
...
async fn run_prompt(
    ...
    let request = match command {
        ...
        Some(SlashCommand::Fork) => {
            runtime
                .request("session.fork", json!({"session_id": session_id}))
                .await
        }
...
async fn fork_acp_session(runtime: &AcpRuntime, source_id: &str) -> Result<ForkSessionResponse> {
    let value = runtime
        .request("session.fork", json!({"session_id": source_id}))
        .await?;
    let id = value
        .get("forked_session_id")
        .and_then(Value::as_str)
        .context("session.fork response omitted forked_session_id")?;
    let options = session_config_options(runtime, id).await?;
...
async fn prepare_observer(
    ...
    if let Some(observer) = observers.get(session_id).cloned() {
        ...
        let snapshot = if replay {
            Some(json!({"snapshot": runtime.request(
                "session.snapshot",
...
async fn handle_session_event(
    ...
    match kind {
        ...
        "config_changed" => {
            ...
            let session_id = session_id.to_string();
            let _ = cx.inner.clone().spawn(async move {
                match session_config_options(&config_runtime, &session_id).await {
                    Ok(options) => {
                        send_update(
                            &cx,
                            &session_id,
...
async fn session_config_options(
    ...
    let value = runtime
        .request("session.options", json!({"session_id": session_id}))
        .await?;
...
async fn send_available_commands(runtime: &AcpRuntime, cx: &AcpConnection, session_id: &str) {
...
async fn load_status_snapshot(
    ...
    let value = runtime
        .request("session.snapshot", json!({"session_id": session_id}))
        .await?;
...
fn send_assistant_interrupted(cx: &AcpConnection, session_id: &str, payload: &Value) {
    ...
    if let (Some(message_id), Some(reasoning)) = (
        ...
        if send_content || cx.protocol == AcpProtocol::V1 {
            let content = if send_content {
                vec![ContentBlock::Text(TextContent::new(reasoning))]
            } else {
                vec![ContentBlock::Text(TextContent::new("\u{200b}"))]
            };
            send_update(
                cx,
                session_id,
