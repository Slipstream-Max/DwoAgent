...
use dwo_command::{parse_command, routes_to_channel_command};
...
pub async fn create_bind_task() -> Result<QqBindTask> {
...
pub fn bind_qr_url(task_id: &str) -> String {
...
pub async fn poll_bind_task(task_id: &str, key: &str) -> Result<QqQrPoll> {
    let response = reqwest::Client::new()
...
pub async fn validate_credentials(app_id: &str, app_secret: &str) -> Result<()> {
    ...
    async fn process_prompt(&self, message: &C2CMessage, text: &str) -> Result<()> {
        ...
        if !message.attachments.is_empty() {
            ...
            for (index, attachment) in message.attachments.iter().enumerate() {
                let path =
                    download_attachment(self.host.as_ref(), &session_id, attachment, index).await?;
